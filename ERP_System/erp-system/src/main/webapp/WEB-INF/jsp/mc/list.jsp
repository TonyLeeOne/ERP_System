...
<li class="x-body">
    ...
    <xblock>
        ...
        <shiro:hasPermission name="material:add">
            <button class="layui-btn" onclick="x_admin_show('添加新领料单','/materialConsume/edit',730,500)"><i
...
<li>
    ...
    * <fmt:formatNumber pattern="#" value="${detail.bdNum*consume.mcCountNeeded*(1+detail.bdRate)}"/>${detail.material.mUnit}
</li>
</c:forEach>
</c:if>
</td>
...
<td>
    <c:if test="${! empty consume.details&& ! empty consume.mcCountIndeed}">
        <c:forEach items="${consume.details}" var="detail">
            <li>
                <i class="layui-icon"
                   style="color: #dd122c">&#xe643;</i>　${detail.material.mName}
                * <fmt:formatNumber pattern="#"
                                    value="${detail.bdNum*consume.mcCountIndeed*(1+detail.bdRate)}"/> ${detail.material.mUnit}
            </li>
        </c:forEach>
    </c:if>
